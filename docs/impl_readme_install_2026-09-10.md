...
2. **Código da versão.** O operador disponibiliza o checkout/release previamente; não há clone de URL não homologada nem atualização automática por `git pull`. Configurações, uploads, logs e snapshots devem ser preservados ao atualizar. Caminho fixo `/opt/sites/glpi-portal` acompanha os caminhos absolutos atuais do core.
